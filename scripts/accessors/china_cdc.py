...
def main():
    """
    Example usage of ChinaCDCAccessor.
    """
    print("=" * 70)
    print("China CDC Weekly Data Accessor")
    print("=" * 70)

    ccdc = ChinaCDCAccessor()

    # List provinces
    print("\n🇨🇳 Chinese Provinces (sample):")
    provinces = ccdc.list_provinces()
    print(f"Total provinces: {len(provinces)}")
    print(provinces.head(10).to_string(index=False))

    # List notifiable diseases
    print("\n🦠 Notifiable Diseases (sample):")
    diseases = ccdc.list_notifiable_diseases()
    print(f"Total diseases: {len(diseases)}")
    print(diseases.head(10).to_string(index=False))

    # Get weekly reports metadata
    print("\n📰 Weekly Reports (sample):")
    reports = ccdc.get_weekly_reports(year=2024, week=1)
    ...
    print("  covid = ccdc.get_covid_updates()")
