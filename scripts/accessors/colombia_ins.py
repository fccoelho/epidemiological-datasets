...
def main():
    """
    Example usage of ColombiaINSAccessor.
    """
    print("=" * 70)
    print("Colombia INS (Instituto Nacional de Salud) Data Accessor")
    print("=" * 70)
    ...
    depts = ins.list_departments()
    print(f"Total departments: {len(depts)}")
    print("\nFirst 10 departments:")
    print(depts.head(10).to_string(index=False))
    ...
    vector_diseases = ins.get_diseases_by_group("ENFERMEDADES TRANSMITIDAS POR VECTORES")
    for code in vector_diseases[:5]:
        print(f"  - {code}: {ins.DISEASE_CODES[code]}")

    # List event groups
    print("\n📋 Event Groups:")
    groups = ins.list_event_groups()
    for group in groups["event_group"].unique():
        n_diseases = len(groups[groups["event_group"] == group])
        print(f"  - {group}: {n_diseases} diseases")

    # Show regions
    print("\n🗺️  Geographical Regions:")
    regions = ["Andina", "Caribe", "Pacífica", "Orinoquía", "Amazonía"]
    for region in regions:
        depts_in_region = ins.get_departments_by_region(region)
        print(f"  - {region}: {len(depts_in_region)} departments")

    # Example data access (commented to avoid API calls)
    # print("\n📊 Dengue Data Example:")
    # dengue = ins.get_dengue_data(
    #     years=[2023],
    #     departments=['05', '76']  # Antioquia, Valle del Cauca
    # )
    # print(dengue.head().to_string())

    print("\n✅ Colombia INS accessor ready to use!")
    print("\nExample usage:")
    print("  ins = ColombiaINSAccessor()")
    print("  dengue = ins.get_dengue_data(")
    print("      years=[2022, 2023],")
    print("      departments=['05', '76']  # Antioquia, Valle")
    print("  )")
    print("\n  malaria = ins.get_malaria_data(years=[2023])")
    print("  bulletins = ins.get_weekly_bulletins(year=2023, week=10)")
