...
class EurostatAccessor:
    ...
    def list_all_countries(self) -> pd.DataFrame:
        ...
        for code, name in self.EU_MEMBER_STATES.items():
            data.append({'country_code': code, 'country_name': name, 'type': 'EU Member'})
        ...
        for code, name in self.CANDIDATE_COUNTRIES.items():
            data.append({'country_code': code, 'country_name': name, 'type': 'Candidate'})
        
        for code, name in self.FORMER_MEMBERS.items():
            data.append({'country_code': code, 'country_name': name, 'type': 'Former EU'})
    ...
    def search_indicators(self, keyword: str) -> pd.DataFrame:
        ...
        mask = (
            indicators['indicator_code'].str.contains(keyword, case=False, na=False) |
    ...
    def get_health_indicator(
        ...
        logger.info(f"Fetching indicator {indicator_code} for {len(countries)} countries")
        
        if self.use_library:
            return self._get_with_library(indicator_code, countries_iso2, years, sex, age_group)
        else:
            return self._get_with_api(indicator_code, countries_iso2, years, sex, age_group)
    ...
    def _to_iso2(self, country_code: str) -> str:
        ...
        mapping = {
            ...
            "CHE": "CH", "GBR": "UK", "ALB": "AL", "MNE": "ME", "MKD": "MK",
            "SRB": "RS", "TUR": "TR", "UKR": "UA", "MDA": "MD", "BIH": "BA",
            "GEO": "GE",
        }
    ...
    def get_self_perceived_health(
        ...
        """
        return self.get_health_indicator('hlth_silc_01', countries, years, sex, age_group)
...
def main():
    """
    Example usage of EurostatAccessor.
    """
    print("=" * 70)
    print("Eurostat Health Data Accessor")
    print("=" * 70)
    ...
    if EUROSTAT_LIB_AVAILABLE:
        print("\n📦 Using eurostat Python library")
    else:
        print("\n🌐 Using REST API (install 'eurostat' library for better performance)")
    ...
    # List all countries
    print("\n🌍 All Countries in Eurostat:")
    all_countries = eurostat.list_all_countries()
    for country_type in all_countries['type'].unique():
        n = len(all_countries[all_countries['type'] == country_type])
        print(f"  - {country_type}: {n} countries")
    ...
    results = eurostat.search_indicators('mortality')
    if not results.empty:
        print(results.to_string(index=False))
    else:
        print("  No results found")
    
    print("\n✅ Eurostat accessor ready to use!")
    print("\nExample usage:")
    print("  eurostat = EurostatAccessor()")
    print("  expenditure = eurostat.get_healthcare_expenditure(")
    print("      countries=['DEU', 'FRA', 'ITA'],")
    print("      years=[2020, 2021, 2022]")
    print("  )")
