...
if __name__ == "__main__":
    ...
    print(hdg.list_datasets())
    print("\n" + "="*60 + "\n")
    
    # Test getting hospital data for California
    print("California Hospital Capacity (last 30 days):")
    from datetime import datetime, timedelta
    end = datetime.now().strftime("%Y-%m-%d")
    start = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d")
    
    try:
        ca_data = hdg.get_hospital_capacity(state="CA", date_range=(start, end))
        print(f"Retrieved {len(ca_data)} records")
        print(ca_data.head())
    except Exception as e:
        print(f"Error: {e}")
