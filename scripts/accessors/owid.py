...
class OWIDAccessor:
    ...
    COUNTRIES = {
        ...
        "COG": "Congo", "CRI": "Costa Rica", "HRV": "Croatia",
        "CUB": "Cuba", "CYP": "Cyprus", "CZE": "Czech Republic",
        "COD": "Democratic Republic of Congo", "DNK": "Denmark",
        ...
        "NAM": "Namibia", "NRU": "Nauru", "NPL": "Nepal",
        ...
        "VAT": "Vatican", "VEN": "Venezuela", "VNM": "Vietnam",
    ...
    def get_covid_data(
        ...
        if metrics:
            ...
            metric_mapping = {
                ...
                'tests': ['total_tests', 'new_tests', 'total_tests_per_thousand', 'new_tests_per_thousand', 'positive_rate'],
                'hospitalizations': ['hosp_patients', 'hosp_patients_per_million', 'icu_patients', 'icu_patients_per_million'],
                'reproduction_rate': ['reproduction_rate'],
                'policy': ['stringency_index', 'containment_health_index', 'economic_support_index']
            }
        ...
        logger.info(f"Retrieved {len(df)} COVID-19 records for {df['country_code'].nunique()} countries")
        return df
    ...
    def get_vaccination_data(
        ...
        logger.info(f"Retrieved {len(df)} vaccination records for {df['country_code'].nunique()} countries")
        return df
    ...
    def get_excess_mortality(
        ...
        logger.info(f"Retrieved {len(df)} excess mortality records for {df['country_code'].nunique()} countries")
        return df
    ...
    def get_testing_data(
        ...
        logger.info(f"Retrieved {len(df)} testing records for {df['country_code'].nunique()} countries")
        return df
    ...
    def get_hospitalizations_data(
        ...
        logger.info(f"Retrieved {len(df)} hospitalization records for {df['country_code'].nunique()} countries")
        return df
    ...
    def get_region_aggregates(
        ...
        if region not in self.REGIONS:
            logger.warning(f"Region '{region}' not found. Available: {list(self.REGIONS.keys())}")
            return pd.DataFrame()
