...
class PAHOAccessor:
    ...
    SUBREGIONS = {
        ...
        "Caribbean": ["ATG", "BHS", "BRB", "CUB", "DMA", "DOM", "GRD", "HTI", "JAM", "KNA", "LCA", "TTO", "VCT"],
    ...
    def get_immunization_coverage(
        ...
        if not valid_vaccines:
            raise ValueError(f"No valid vaccine codes. Available: {list(self.VACCINES.keys())}")
        
        # Use WHO Immunization API as the data source
        # This is the same data that PAHO uses
        logger.info(f"Fetching immunization data for {len(valid_vaccines)} vaccines, "
    ...
    def get_malaria_incidence(
        ...
        if not countries:
            ...
            countries = ['BRA', 'COL', 'ECU', 'VEN', 'GUY', 'SUR', 'PER', 'BOL', 
                        'HND', 'NIC', 'GTM', 'SLV', 'CRI', 'PAN', 'DOM', 'HTI']
...
def main():
    """
    Example usage of PAHOAccessor.
    """
    print("=" * 70)
    print("PAHO (Pan American Health Organization) Data Accessor")
    print("=" * 70)
    ...
    for subregion in subregions['subregion'].unique():
        ...
        print(f"  - {subregion}: {n_countries} countries")
    
    # List vaccines
    print("\n💉 Available Vaccines:")
    vaccines = paho.list_vaccines()
    print(vaccines.to_string(index=False))
    
    # Example: Get immunization coverage (commented to avoid API calls)
    # print("\n📊 DTP3 Immunization Coverage (Southern Cone, 2020-2022):")
    # coverage = paho.get_immunization_coverage(
    #     vaccines=['DTP3'],
    #     subregion='Southern Cone',
    #     years=[2020, 2021, 2022]
    # )
    # print(coverage.head().to_string())
    
    print("\n✅ PAHO accessor ready to use!")
    print("\nExample usage:")
    print("  paho = PAHOAccessor()")
    print("  coverage = paho.get_immunization_coverage(")
    print("      vaccines=['DTP3', 'MCV1'],")
    print("      subregion='Southern Cone',")
