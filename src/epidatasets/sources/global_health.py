...
class GlobalHealthAccessor(BaseAccessor):
    """
    ...
    source_name: ClassVar[str] = "global_health"
    source_description: ClassVar[str] = "Global.health - standardized line-list data for pandemics and epidemics (COVID-19, Mpox)"
    source_url: ClassVar[str] = "https://global.health/"
    ...
    def list_countries(self) -> pd.DataFrame:
        all_countries = set()
        for disease, info in self.DATASETS.items():
            try:
                metadata = self.get_outbreak_metadata(disease)
                if metadata.get("countries"):
                    all_countries.update(metadata["countries"])
            except Exception:
                pass
        if all_countries:
            return pd.DataFrame(
                [(c, c) for c in sorted(all_countries)],
                columns=["country_code", "country_name"],
            )
        return pd.DataFrame(columns=["country_code", "country_name"])
