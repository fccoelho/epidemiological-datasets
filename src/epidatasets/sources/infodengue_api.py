...
class InfoDengueAPI(BaseAccessor):
    """
    ...
    source_name: ClassVar[str] = "infodengue"
    source_description: ClassVar[str] = (
        "Epidemiological data from the InfoDengue project, which monitors "
        "dengue, chikungunya, and Zika cases in Brazil via the Mosqlimate API."
    )
    source_url: ClassVar[str] = "https://api.mosqlimate.org/"
    ...
    def list_countries(self) -> pd.DataFrame:
        """
        List countries covered by InfoDengue.

        InfoDengue only covers Brazil, returning Brazilian states.
