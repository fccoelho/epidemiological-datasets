...
class PathoplexusAccessor(BaseAccessor):
    """
    ...
    source_name: ClassVar[str] = "pathoplexus"
    source_description: ClassVar[str] = (
        "Pathoplexus genomic pathogen database providing access to "
        "human viral pathogen genomic sequences, metadata, mutations, "
        "and insertions via the LAPIS API."
    )
    source_url: ClassVar[str] = "https://pathoplexus.org"
    ...
    def list_countries(self) -> pd.DataFrame:
        """Return countries available in Pathoplexus for the current organism."""
        try:
